...
void rsa_gen_keys(struct public_key_class* pub, struct private_key_class* priv)
{
    ...
    do {
        int bytes_read = fread(buffer, 1, static_cast<int>(sizeof(buffer)) - 1, primes_list);
        buffer[bytes_read] = '\0';
        for (i = 0; buffer[i]; i++) {
            if (buffer[i] == '\n') {
                prime_count++;
    ...
    do {
        ...
        int a = static_cast<int>(static_cast<double>(rand()) / (RAND_MAX + 1.0) * (prime_count + 1));
        int b = static_cast<int>(static_cast<double>(rand()) / (RAND_MAX + 1.0) * (prime_count + 1)) ;
